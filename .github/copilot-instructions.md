...
- `DatasourceValidator` (`config/datasource/`) fails fast at startup if `app.datasource` is not in `Datasources.ALLOWED_DATASOURCES` (`postgres`, `mongo`). Extend this list plus `Datasources` constants when adding a new backend.
